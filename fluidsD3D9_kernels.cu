...
void unbindTexture(void)
{
    ...
    getLastCudaError("cudaUnbindTexture failed");
}
...
{
    cudaFreeArray(array);
    getLastCudaError("cudaFreeArray failed");
}
...
extern "C"
void addForces(cData *v, int dx, int dy, int spx, int spy, float fx , float fy, int r, size_t tPitch)
{
...
void advectVelocity(cData *v, float *vx, float *vy,
                    int dx, int pdx, int dy, float dt, size_t tPitch)
{
...
void updateVelocity(cData *v, float *vx, float *vy,
                    int dx, int pdx, int dy, size_t tPitch)
{
    dim3 grid((dx/TILEX)+(!(dx%TILEX)?0:1), (dy/TILEY)+(!(dy%TILEY)?0:1));
...
extern "C"
void advectParticles(Vertex *p, cData *v, int dx, int dy, float dt, size_t tPitch)
{
    dim3 grid((dx/TILEX)+(!(dx%TILEX)?0:1), (dy/TILEY)+(!(dy%TILEY)?0:1));
